...
export function CartDrawer() {
  ...
  const handleWhatsApp = () => {
    const lines = items.map((i) => {
      const preco = i.precoPromocional ?? i.preco;
      return `• ${i.nome} (x${i.quantity}) — R$ ${(preco * i.quantity).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`;
    });
    const msg = [
                  ...
                  <div className="flex flex-1 flex-col justify-between min-w-0">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      <p className="text-sm font-bold text-foreground">
                        R${" "}
                        {((item.precoPromocional ?? item.preco) * item.quantity).toLocaleString("pt-BR", {
        ...
        {items.length > 0 && (
          <div className="border-t border-border px-5 py-5 space-y-4">
            ...
            <Link href="/checkout" onClick={closeCart} className="block">
              <Button className="w-full font-semibold py-5 text-base">
                Finalizar compra
              </Button>
            </Link>
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 rounded-md border border-green-500 bg-white text-green-700 hover:bg-green-50 active:bg-green-100 font-semibold py-3 text-base transition-colors"
            >
